...
object HookStatue {
    ...
    fun Context.getStatue(): Statue {
        if (fakeTaichi(this)) {
            return if (isEnabled()) Statue.taichi_magisk_active else Statue.taichi_active
        }
        val isInstall = isInstall(this)
        val getMagiskModule = GetMagiskModule()
    ...
    fun fakeTaichi(context: Context): Boolean {
        return context.getSharedPreferenceOnUI().getBoolean("fakeTaichi", false)
    }
