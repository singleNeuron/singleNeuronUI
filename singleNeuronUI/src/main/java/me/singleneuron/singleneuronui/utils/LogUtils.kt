...
import me.singleneuron.singleneuronui.sharedpreferences.ContentProvider
import me.singleneuron.singleneuronui.sharedpreferences.ContentProviderPreference
import java.io.File
...
object LogUtils {
    ...
    fun addLogByContentProvider(configName: String, packageName: String, context: Context, applicationId: String, contentProviderUri: String) {
        if (packageName.contains("android.ext.services", true)) return
        val string: String = SimpleDateFormat(
            "[yyyy-MM-dd HH:mm:ss] ",
            Locale.getDefault()
        ).format(Date()) + configName + " apply to " + packageName + "  \n"
        ContentProviderPreference(ContentProvider.CONTENT_PROVIDER_COMMIT, string, context, applicationId, contentProviderUri)
    }
    ...
    fun getLineCount(context: Context): Int {
        ...
        return Integer.parseInt(
            lineResult.out[0].replace(logFile.absolutePath, "").replace(" ", "")
        )
    }
